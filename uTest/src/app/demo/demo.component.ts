...
export class DemoComponent implements OnInit {
  ...
  keyUp(){
  	...
  	if(comments.value.trim().length>0){
		...
		var mat_label = addNode.children[0].children[0].children[0].lastElementChild.children[0].children[0];
		console.log(mat_label);
		mat_label.innerHTML = labelInf;
		discuss.appendChild(addNode);
		var mat_textarea = addNode.children[0].children[0].children[0].children[0];
